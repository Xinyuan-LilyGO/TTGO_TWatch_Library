...
void readTest(void)
{
    ...
    Serial.println(watch.readcommand8(ILI9341_RDID4, 2), HEX);
    Serial.println(watch.readcommand16(ILI9341_RDID4, 2), HEX);
    Serial.println(watch.readcommand32(ILI9341_RDID4, 0), HEX);
}
...
uint32_t readRegister(uint8_t reg, int16_t bytes, uint8_t index)
{
    ...
    while (bytes > 0) {
        bytes--;
        data = (data << 8) | watch.readcommand8(reg, index);
        index++;
    }
