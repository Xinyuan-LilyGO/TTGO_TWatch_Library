...
void setup()
{
    ...
    watch.begin();
    watch.setRotation(0); // Must be setRotation(0) for this sketch to work correctly
    ...
    watch.fillRect(0, 0, 240, 16, TFT_BLUE);
    watch.drawCentreString(" Serial Terminal - 9600 baud ", 120, 0, 2);
...
void loop(void)
{
    ...
    //  if (change_colour){
    //  change_colour = 0;
    //  if (selected == 1) {watch.setTextColor(TFT_CYAN, TFT_BLACK); selected = 0;}
    //  else {watch.setTextColor(TFT_MAGENTA, TFT_BLACK); selected = 1;}
    //}
    ...
    while (Serial.available()) {
        ...
        if (data > 31 && data < 128) {
            xPos += watch.drawChar(data, xPos, yDraw, 2);
...
int scroll_line()
{
    ...
    watch.fillRect(0, yStart, blank[(yStart - TOP_FIXED_AREA) / TEXT_HEIGHT], TEXT_HEIGHT, TFT_BLACK);
...
void setupScrollArea(uint16_t tfa, uint16_t bfa)
{
    ...
    watch.writedata(YMAX - tfa - bfa);
    watch.writedata(bfa >> 8);           // Bottom Fixed Area line count
    watch.writedata(bfa);
}
