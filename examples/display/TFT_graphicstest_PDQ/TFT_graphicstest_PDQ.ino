...
void loop(void)
{
    ...
    uint16_t c = 4;
    int8_t d = 1;
    for (int32_t i = 0; i < watch.height(); i++) {
        watch.drawFastHLine(0, i, watch.width(), c);
        c += d;
        if (c <= 4 || c >= 11)
            d = -d;
    }
    ...
    watch.setTextSize(1);
    watch.println(F(""));
    watch.setTextColor(watch.color565(0x80, 0x80, 0x80));

    watch.println(F(""));


    watch.setTextColor(TFT_GREEN);
    watch.println(F(" Benchmark               microseconds"));
    watch.println(F(""));
    watch.setTextColor(TFT_YELLOW);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("HaD pushColor      "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecHaD);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Screen fill        "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    ...
    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Horiz/Vert Lines   "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecFastLines);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Rectangles         "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecRects);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Rectangles-filled  "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecFilledRects);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Circles            "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecCircles);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Circles-filled     "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecFilledCircles);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Triangles          "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecTriangles);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Triangles-filled   "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecFilledTrangles);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Rounded rects      "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usecRoundRects);

    watch.setTextColor(TFT_CYAN); watch.setTextSize(1);
    watch.print(F("Rounded rects-fill "));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    printnice(usedFilledRoundRects);

    watch.setTextSize(1);
    watch.println(F(""));
    watch.setTextColor(TFT_GREEN); watch.setTextSize(2);
    watch.print(F("Benchmark Complete!"));
...
uint32_t testHaD()
{
    ...
    for (int i = 0; i < 0x10; i++) {
        watch.setAddrWindow(0, 0, 240, 320);

        uint16_t cnt = 0;
        uint16_t color = watch.color565((i << 4) | i, (i << 4) | i, (i << 4) | i);
        uint16_t curcolor = 0;
        ...
        while (cmp < &HaD_240x320[sizeof(HaD_240x320)]) {
            ...
            watch.pushColor(curcolor, cnt);   // PDQ_GFX has count
            curcolor ^= color;
        }
        watch.endWrite();
    }
    ...
    watch.setTextColor(TFT_YELLOW);
    watch.setTextSize(2);
    watch.setCursor(8, 285);
    watch.print(F("http://hackaday.io/"));
    watch.setCursor(96, 302);
    watch.print(F("Xark"));
...
uint32_t testText()
{
    ...
    watch.println(F("Hello World!"));
    watch.setTextSize(2);
    watch.setTextColor(watch.color565(0xff, 0x00, 0x00));
    watch.print(F("RED "));
    watch.setTextColor(watch.color565(0x00, 0xff, 0x00));
    watch.print(F("GREEN "));
    watch.setTextColor(watch.color565(0x00, 0x00, 0xff));
    watch.println(F("BLUE"));
    watch.setTextColor(TFT_YELLOW); watch.setTextSize(2);
    ...
    watch.setTextColor(TFT_GREEN);
    watch.setTextSize(5);
    watch.println(F("Groop"));
    watch.setTextSize(2);
    watch.println(F("I implore thee,"));
    watch.setTextColor(TFT_GREEN);
    watch.setTextSize(1);
    watch.println(F("my foonting turlingdromes."));
    watch.println(F("And hooptiously drangle me"));
    watch.println(F("with crinkly bindlewurdles,"));
    watch.println(F("Or I will rend thee"));
    watch.println(F("in the gobberwarts"));
    watch.println(F("with my blurglecruncheon,"));
    watch.println(F("see if I don't!"));
    watch.println(F(""));
    watch.println(F(""));
    watch.setTextColor(TFT_MAGENTA);
    watch.setTextSize(6);
    watch.println(F("Woot!"));
    uint32_t t = micros() - start;
    delay(1000);
    return t;
}
...
uint32_t testPixels()
{
    int32_t w = watch.width();
    int32_t h = watch.height();

    uint32_t start = micros_start();
    watch.startWrite();
    for (uint16_t y = 0; y < h; y++) {
        for (uint16_t x = 0; x < w; x++) {
            watch.drawPixel(x, y, watch.color565(x << 3, y << 3, x * y));
        }
    }
    watch.endWrite();
    return micros() - start;
...
uint32_t testLines(uint16_t color)
{
    ...
    for (x2 = 0; x2 < w; x2 += 6) {
        watch.drawLine(x1, y1, x2, y2, color);
    }
...
uint32_t testFastLines(uint16_t color1, uint16_t color2)
{
    ...
    for (y = 0; y < h; y += 5)
        watch.drawFastHLine(0, y, w, color1);
    for (x = 0; x < w; x += 5)
        watch.drawFastVLine(x, 0, h, color2);
...
uint32_t testRects(uint16_t color)
{
    uint32_t start;
    int32_t n, i, i2;
    int32_t cx = watch.width() / 2;
    int32_t cy = watch.height() / 2;
...
uint32_t testFilledRects(uint16_t color1, uint16_t color2)
{
    uint32_t start, t = 0;
    int32_t n, i, i2;
    int32_t cx = watch.width() / 2 - 1;
    int32_t cy = watch.height() / 2 - 1;
...
{
    uint32_t start;
    int32_t x, y, w = watch.width(), h = watch.height(), r2 = radius * 2;
...
uint32_t testCircles(uint8_t radius, uint16_t color)
{
    ...
    int32_t w = watch.width() + radius;
    int32_t h = watch.height() + radius;
