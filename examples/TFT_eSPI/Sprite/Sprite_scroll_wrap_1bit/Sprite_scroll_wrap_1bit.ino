...
TFT_eSPI *tft = NULL;
TFT_eSprite *fb =  NULL;  // Sprite object for frame buffer
TFT_eSprite *img =  NULL;  // Sprite object for frame buffer
...
void setup()
{
    ...
    img   = new TFT_eSprite(tft); // Sprite object
    fb   = new TFT_eSprite(tft); // Sprite object
    ...
    img->setColorDepth(1);
    gfxPtr = (uint16_t *) img->createSprite(WIDTH, HEIGHT);     // 450 bytes needed
    ...
    img->pushSprite(0, 0); // Plot to screen so we see what it looks like
    delay(2000);
}
...
{
    img->fillSprite(TFT_BLACK);           // Clear sprite each time and completely redraw

    //img->drawRect(0,0,60,60,TFT_WHITE); // Test to check alignment

    img->drawCircle( 30, 30, radius, TFT_WHITE);
    if (grow) radius++;
    else radius--;
    ...
    img->drawString("World", 30, 40, 2);   // Plot hello in frame buffer
}
...
void wrappingScroll(int16_t dx, int16_t dy)
{
    ...
    // push the 4 quadrants of img-> sprite into the fb-> sprite
