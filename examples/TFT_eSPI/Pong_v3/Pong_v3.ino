...
void setup(void)
{
    ...
    ttgo->eTFT->fillScreen(BLACK);

    initgame();

    ttgo->eTFT->setTextColor(WHITE, BLACK);
...
void initgame()
{
    ...
    ttgo->eTFT->fillRect(0, h - 26, w, h - 1, BLACK);

    ttgo->eTFT->setTextDatum(TC_DATUM);
    ttgo->eTFT->setTextColor(WHITE);
    ttgo->eTFT->drawString("TFT_eSPI example", w / 2, h - 26, 2);
}
...
void midline()
{
    ...
    ttgo->eTFT->setAddrWindow(dashline_x, 0, dashline_w, h);

    for (int16_t i = 0; i < dashline_n; i += 2) {
        ttgo->eTFT->pushColor(WHITE, dashline_w * dashline_h); // push dash pixels
        ttgo->eTFT->pushColor(BLACK, dashline_w * dashline_h); // push gap pixels
    }
...
void lpaddle()
{
    ...
    if (lpaddle_d == 1) {
        ttgo->eTFT->fillRect(lpaddle_x, lpaddle_y, paddle_w, 1, BLACK);
    } else if (lpaddle_d == -1) {
        ttgo->eTFT->fillRect(lpaddle_x, lpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
    }
    ...
    ttgo->eTFT->fillRect(lpaddle_x, lpaddle_y, paddle_w, paddle_h, WHITE);
}
...
void rpaddle()
{
    ...
    if (rpaddle_d == 1) {
        ttgo->eTFT->fillRect(rpaddle_x, rpaddle_y, paddle_w, 1, BLACK);
    } else if (rpaddle_d == -1) {
        ttgo->eTFT->fillRect(rpaddle_x, rpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
    }
    ...
    ttgo->eTFT->fillRect(rpaddle_x, rpaddle_y, paddle_w, paddle_h, WHITE);
}
...
void ball()
{
    ...
    //ttgo->eTFT->fillRect(oldball_x, oldball_y, ball_w, ball_h, BLACK);
    ttgo->eTFT->drawRect(oldball_x, oldball_y, ball_w, ball_h, BLACK); // Less TFT refresh aliasing than line above for large balls
    ttgo->eTFT->fillRect(   ball_x,    ball_y, ball_w, ball_h, WHITE);
    oldball_x = ball_x;
    oldball_y = ball_y;
}
